...
const MONGO_URI='mongodb+srv://oscar-rangel_93:Eren_93@cluster0-p8j3h.mongodb.net/test?retryWrites=true&w=majority';

_mongoose.connect(MONGO_URI,{ useNewUrlParser: true, useUnifiedTopology: true }); 
...
const Person = _mongoose.model("Person", personSchema);
...
const createAndSavePerson = function(done) {
  const janeDoe= new Person({name: "Jane Doe", age: 24, favoriteFoods: ["spaghetti", "fried rice"]});
...
let arrayOfPeople = [
  {name: "Jon", age: 32, favoriteFoods: ["tacos"]},
  {name: "Sansa", age: 26, favoriteFoods: ["roast chicken"]},
  {name: "Lagertha", age: 52, favoriteFoods: ["wine"]}];

const createManyPeople = function(arrayOfPeople, done) {
    Person.create(arrayOfPeople,(error, people) => {
      return error ? console.error(error) : done(null, people);      
    });
};
...
const findPeopleByName = function(personName, done) {
  Person.find({name: personName}, function (err, personFound) {
    return err ? console.error(err) : done(null,personFound);
  });
};
...
const findOneByFood = function(food, done) {
  Person.findOne({favoriteFoods: food},(error,target)=>{
    return error ? console.error(error) : done(null, target);
  });
};
...
const findPersonById = function(personId, done) {
  Person.findById(personId, (error, target)=>{
    return error ? console.error(error) : done(null, target);
  });
};
...
const findEditThenSave = function(personId, done) {
  ...
  Person.findById(personId, (error, target)=>{
    target.favoriteFoods.push(foodToAdd);
    target.save((error, data)=>{
      return error ? console.error(error) : done(null, data);
    });
  });
...
const removeManyPeople = function(done) {
  ...
  Person.remove({'name':nameToRemove},(error,data)=>{
    return error ? console.error(`Error: ${error}`) : done(null, data);
  });
};
